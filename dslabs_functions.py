...
def plot_bar_chart(xvalues: list, yvalues: list, ax: Axes = None, title: str = '', xlabel: str = '',
    ...
    values = ax.bar(xvalues, yvalues, label=yvalues,
                    edgecolor=LINE_COLOR, color=FILL_COLOR, tick_label=xvalues)
...
def plot_multibar_chart(group_labels: list, yvalues: dict, ax: Axes = None, title: str = '',
    ...
    for i in range(len(bar_labels)):
        values = ax.bar(index + i * bar_width,
                        yvalues[bar_labels[i]], width=bar_width, label=bar_labels[i])
        format = '%.2f' if percentage else '%.0f'
        ax.bar_label(values, fmt=format, fontproperties=FONT_TEXT)
        ...
        if len(values) > 2:
            ...
            for i in range(len(values)):
                ...
                ax.scatter(subset[var1], subset[var2],
                           color=ACTIVE_COLORS[i], label=values[i])
...
def count_outliers(data: DataFrame, numeric, nrstdev: int = NR_STDEV, iqrfactor: float = IQR_FACTOR):
    ...
    for var in numeric:
        ...
        outliers_stdev += [data[data[var] > top].count()
                           [var] + data[data[var] < bottom].count()[var]]

        top, bottom = determine_outlier_thresholds_for_var(
            summary5[var], std_based=False, threshold=iqrfactor)
        outliers_iqr += [data[data[var] > top].count()[var] +
                         data[data[var] < bottom].count()[var]]
...
def analyse_date_granularity(data, var, levels, file_tag=''):
    ...
    for i in range(cols):
        ...
        plot_bar_chart(counts.index.to_list(), counts.values,
                       ax=axs[0][i], title=levels[i], xlabel=levels[i], ylabel='nr records', percentage=False)
...
def dummify(df, vars_to_dummify):
    ...
    enc = OneHotEncoder(handle_unknown='ignore',
                        sparse_output=False, dtype=bool, drop='if_binary')
...
def mvi_by_filling(data: DataFrame, strategy: str = 'frequent') -> DataFrame:
    ...
    if strategy != 'knn':
        ...
        if len(variables['numeric']) > 0:
            ...
            tmp_nr = DataFrame(imp.fit_transform(
                data[variables['numeric']]), columns=variables['numeric'])
        if len(variables['symbolic']) > 0:
            imp = SimpleImputer(strategy=stg_sym, fill_value=v_sym, copy=True)
            tmp_sb = DataFrame(imp.fit_transform(
                data[variables['symbolic']]), columns=variables['symbolic'])
        if len(variables['binary']) > 0:
            imp = SimpleImputer(strategy=stg_bool,
                                fill_value=v_bool, copy=True)
            tmp_bool = DataFrame(imp.fit_transform(
                data[variables['binary']]), columns=variables['binary'])
...
def plot_evaluation_results(model, trn_y, prd_trn, tst_y, prd_tst, labels: ndarray, file_tag=''):
    ...
    for key in CLASS_EVAL_METRICS:
        evaluation[key] = [CLASS_EVAL_METRICS[key](
            trn_y, prd_trn), CLASS_EVAL_METRICS[key](tst_y, prd_tst)]
    ...
    plot_multibar_chart(['Train', 'Test'], evaluation,
                        ax=axs[0], percentage=True)

    cnf_mtx_tst = confusion_matrix(tst_y, prd_tst, labels=labels)
    plot_confusion_matrix(cnf_mtx_tst, labels, ax=axs[1])
    savefig(f'images/{file_tag}_{model['name']
                                 }_best_{model['metric']}_eval.png')
    return axs
...
def naive_Bayes_study(trnX, trnY, tstX, tstY, metric='accuracy', file_tag=''):
    ...
    plot_bar_chart(xvalues, yvalues, title=f'Naive Bayes Models ({
                   metric})', ylabel=metric, percentage=True)
...
def knn_study(trnX, trnY, tstX, tstY, k_max=19, lag=2, metric='accuracy', file_tag=''):
    ...
    print(f'KNN best with k={best_params['params'][0]} and {
          best_params['params'][1]}')

    plot_multiple_line_chart(kvalues, values, title=f'KNN Models ({
                             metric})', xlabel='k', ylabel=metric, percentage=True)
